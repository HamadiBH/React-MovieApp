...
class Movieapp extends React.Component {
  ...
  render() {
   const filteredMovies = this.state.movies.filter(movie =>(movie.name.toLowerCase().includes(this.state.search.toLowerCase().trim()) && movie.rating >= this.state.rating) )
  console.log(this.state.movies)
   return (
    <div className="App">
​
      <Searchbar handleSearch={this.handleSearch} count={this.state.rating} handleChangeRating={this.handleChangeRating}/>
      <Moviecards movies={filteredMovies} handleChangeRating={this.handleChangeRating} addMovie={this.addMovie} />
